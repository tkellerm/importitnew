...
import de.abaspro.infosystem.importit.Main;
...
public abstract class AbstractDataProcessing implements AbasDataProcessable {
	...
	protected EDPSession edpSession;
	protected Logger logger = Logger.getLogger(Main.class);


	protected abstract void writeData(Data data) throws ImportitException;
	protected abstract boolean checkDataStructure(Data data);
	
	

	public AbstractDataProcessing(String server, Integer port, String client,
			String password) {
	...
	@Override
	public void importDataList(ArrayList<Data> dataList)
			throws ImportitException {
		edpSession.loggingOn("java/log/importit21edp.log");
        try {
            startEdpSession();
	...
	public void abortTransaction() throws ImportitException {
		...
		try {
			...
			throw new ImportitException(
					Util.getMessage("err.transaction.cancel"), e);

		} catch (ConnectionLostException e) {
			throw new ImportitException(
					Util.getMessage("err.edp.connection.cancel"), e);
		} finally {
			closeEdpSession(this.edpSession);
		}
	...
	public void commitTransaction() throws ImportitException {
		try {
			...
			throw new ImportitException(
					Util.getMessage("err.transaction.commit"), e);
		} finally {
			closeEdpSession(this.edpSession);
		}
	}
	...
	public void startEdpSession(EDPVariableLanguage variableLanguage)
			throws ImportitException {
		createSession(this.server, this.port, this.client, this.password,
				variableLanguage);
	}

	private void createSession(String server, int port, String client,
			String password, EDPVariableLanguage variableLanguage)
			throws ImportitException {
		try {
			...
			logger.error(e);
			throw new ImportitException(
					Util.getMessage("err.edp.session.start", e));
		}
	}

	protected EDPSession connectNewEdpSession(
	...
	private void createSession(String server, int port, String client,
			String password) throws ImportitException {
		try {
			this.edpSession.beginSession(server, port, client, password,
					"ImportIt_21");
			this.edpSession.loggingOn(edpLogFile);
    ...
    protected void setEditorOption(Data data, EDPEditor edpEditor) throws CantChangeSettingException, CantReadSettingException {
    ...
    protected String getSelObject(String criteria, Data data) throws ImportitException, InvalidQueryException {
    ...
    protected boolean getAbasType(List<Field> fieldList, Integer database, Integer group, Boolean inTab, Boolean englishVariables) throws ImportitException {
        if (fieldList != null) {
            if (!edpSession.isConnected()) {
                startEdpSession(EDPVariableLanguage.ENGLISH);
            }
