...
import de.abas.erp.db.infosystem.custom.ow1.InfosystemImportit.Row;
...
//		doku-Verzeichnis in win/tmp/ kopieren und danach den Browser �ffnen
			InfosystemImportit infosysImportit = event.getSourceRecord();
...
//						file://///cebitmaster40/entw-fepco/owdoku/docu/Dokumentation.html
	...
	class PruefDatenButtonListener extends ButtonListenerAdapter<InfosystemImportit>{
	    ...
	    public void after(ButtonEvent<InfosystemImportit> event) throws EventException {
	      super.after(event);
	      logger.debug("Start pr�fen der Daten");
	      String name = logger.getName();
	      Level loglevel = logger.getLevel();
	      ypruefdatButtonInvoked(event);
	      logger.debug("Ende pr�fen der Daten");
	    }
		...
		private void ypruefdatButtonInvoked(
			...
			if (checkDatensatzListNotEmpty()) {
				try {
					...
					if (infosysImportit.getYfehlerstruktur() == 0 ) {
						...
						throw new ImportitException("Es sind noch Fehler aus der Strukturpr�fung vorhanden! Bitte zuerst beheben!");
					}
			...
			if (infosysImportit.getYfehlerdatpruef() == 0) {
				infosysImportit.setYstatus("Datenpr�fung erfolgreich");
			}else {
				infosysImportit.setYstatus("Datenpr�fung fehlerhaft");
			}
			
			TextBox textbox = new TextBox(getContext(), "Fertig", "Datenpr�fung abgeschlossen!");
			textbox.show();
		}
				...
				try {
					if (checkDatensatzListNotEmpty()) {
						...
						if (infosysImportit.getYdatafile().isEmpty()) {
							...
							TextBox textBox = new TextBox(getContext(),
									"Fehler",
									"Es wurde f�r die Excel-Datei noch keine Strukturpr�fung durchgef�hrt.");
							textBox.show();
						}
					}
				} catch (ImportitException e) {
...
//	    		pr�fe noch ob passwort eingeben wurde  
				...
				showOptions(infosysImportit , datensatzList);
				if (infosysImportit.getYfehlerstruktur() == 0) {
					infosysImportit.setYstatus("Strukturpr�fung durchgelaufen");
				}else {
					infosysImportit.setYstatus("Strukturpr�fung fehlerhaft");
				}
				TextBox textbox = new TextBox(getContext(), "Fertig", "Strukturpr�fung abgeschlossen!");
				textbox.show();
	...
	/**
	 * �berpr�ft ob die Datensatzliste gef�llt ist.
	 * 
	 * @return true wenn Datesatz List gef�llt ist.
	 */
